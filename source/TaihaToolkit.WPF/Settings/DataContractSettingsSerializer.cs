...
{
	public class DataContractSettingsSerializer : ISettingsSerializer
	{
		static ILogger Logger { get; } = LoggingService.Current.GetLogger(typeof(DataContractSettingsSerializer));
		...
		public void Serialize(Stream stream, SettingsImpl settings)
		{
		...
		object DeserializSeserializeInfo(KeyValueSerializationInfo info, IEnumerable<Type> knownTypes)
		{
			...
			// 型が取得できなかったらnullを返す
			if (type == null) {
				Logger.ErrorFormat("設定取得に失敗しました。(キー:{0}, 型:{1})", new object[] { info.Key, info.Type });
				return null;
			}
